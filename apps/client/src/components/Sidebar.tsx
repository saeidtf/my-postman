...
export function Sidebar({ items, selectedId, history, onCreate, onSelect }: SidebarProps) {
  return (
    <aside className="sidebar">
      <div className="brand-block">
        <p className="eyebrow">API Workbench</p>
        <h1>My Postman</h1>
        <p className="muted">Request builder, environment variables and execution history in one workspace.</p>
      </div>

      <button className="primary-button full-width" onClick={onCreate} type="button">
        New Request
      </button>
        ...
        <div className="history-list">
          {history.map((entry) => (
            <div className="history-card" key={entry.id}>
              <span className={`status-dot ${entry.status < 400 ? "ok" : "fail"}`} />
              <div>
